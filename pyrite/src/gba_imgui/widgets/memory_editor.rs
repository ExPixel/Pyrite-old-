...
impl MemoryEditorWindow {
    ...
    fn calc_sizes(&mut self, sizes: &mut Sizes, mem_size: AddrType, base_display_addr: AddrType) {
        ...
        if self.opts.show_ascii {
            ...
            if self.opts.mid_cols_count > 0 {
                sizes.pos_ascii_start += ((self.columns + self.opts.mid_cols_count - 1) / self.opts.mid_cols_count) as f32 * sizes.spacing_between_mid_cols;
            }
            sizes.pos_ascii_end = sizes.pos_ascii_start + self.columns as f32 * sizes.glyph_width;
        }
        
        sizes.window_width = sizes.pos_ascii_end + style.ScrollbarSize + style.WindowPadding.x * 2.0 + sizes.glyph_width;
    }

    pub fn draw(&mut self, memory: &dyn ArmMemory, mem_size: AddrType, base_display_addr: AddrType) {
        ...
        if imgui::begin(imgui::str!("Memory Editor"), &mut self.open, imgui::WindowFlags::NoScrollbar) {
            ...
            if self.contents_width_changed {
                self.calc_sizes(&mut sizes, mem_size, base_display_addr);
                imgui::set_window_size(imgui::vec2(sizes.window_width, imgui::get_window_size().y), imgui::none());
            }
    ...
    fn draw_contents(&mut self, memory: &dyn ArmMemory, mem_size: AddrType, base_display_addr: AddrType, sizes: &Sizes) {
        let style = imgui::get_style().expect("failed to get style");
        ...
        if self.opts.show_data_preview {
            footer_height += height_separator + imgui::get_frame_height_with_spacing() * 1.0 + imgui::get_text_line_height_with_spacing() * 3.0;
        }

        if imgui::is_window_focused(imgui::none()) && !imgui::is_any_item_focused() {
            if imgui::is_key_pressed(imgui::get_key_index(imgui::Key::DownArrow), true) { self.cursor_addr = self.cursor_addr.wrapping_add(self.columns); }
            if imgui::is_key_pressed(imgui::get_key_index(imgui::Key::UpArrow), true) { self.cursor_addr = self.cursor_addr.wrapping_sub(self.columns); }
            if imgui::is_key_pressed(imgui::get_key_index(imgui::Key::RightArrow), true) { self.cursor_addr = self.cursor_addr.wrapping_add(1); }
            if imgui::is_key_pressed(imgui::get_key_index(imgui::Key::LeftArrow), true) { self.cursor_addr = self.cursor_addr.wrapping_sub(1); }
        ...
        imgui::begin_child(imgui::str!("##memory"), imgui::vec2(0.0, -footer_height), false, imgui::WindowFlags::NoMove | imgui::WindowFlags::NoScrollbar);
        ...
        if available_height < 0.0 { available_height = 0.0; }
        ...
        if self.cursor_addr < visible_start_addr {
            visible_start_addr = self.cursor_addr.wrapping_sub(self.cursor_addr % self.columns);
            ...
            visible_end_addr = self.cursor_addr.wrapping_sub(self.cursor_addr % self.columns);
        ...
        if self.opts.show_ascii {
            ...
            draw_list.add_line(
                imgui::vec2(window_pos.x + sizes.pos_ascii_start - sizes.glyph_width, window_pos.y),
                imgui::vec2(window_pos.x + sizes.pos_ascii_start - sizes.glyph_width, window_pos.y + window_size.y + 16.0),
                imgui::get_color_u32(imgui::Col::Border, 1.0),
                1.0
            );
        }
        ...
        for _row in 0..=visible_rows {
            ...
            for col in 0..self.columns {
                ...
                if self.opts.mid_cols_count > 0 {
                    byte_pos_x += (col / self.opts.mid_cols_count) as f32 * sizes.spacing_between_mid_cols;
                }
                ...
                if is_highlight_from_user_range || is_highlight_from_cursor {
                    ...
                    let is_next_byte_highlighted = (addr + 1 < mem_size) && (self.highlight_max != ADDR_MAX && addr + 1 < self.highlight_max);

                    if is_next_byte_highlighted || (col + 1 == self.columns) {
                        highlight_width += sizes.spacing_between_mid_cols;
                    }

                    draw_list.add_rect_filled(pos, imgui::vec2(pos.x + highlight_width, pos.y + sizes.line_height), self.highlight_color);
                }
            ...
            if self.opts.show_ascii {
                ...
                for col in 0..self.columns {
                    ...
                    draw_list.add_text(
                        pos,
                        color,
                        unsafe { imgui::imstr::ImStr::from_bytes_with_nul_unchecked(&buf) });
        ...
        if self.opts.show_options {
            ...
            if imgui::begin_popup(imgui::str!("context"), imgui::none()) {
                ...
                if imgui::drag_int(imgui::str!("##cols"), &mut columns, Some(0.2), Some(4), Some(32), Some(imgui::str!("%d cols"))) {
                    self.columns = std::cmp::max(columns, 0) as u32;
                    self.contents_width_changed = true;
                }
                imgui::pop_item_width();
                imgui::checkbox(imgui::str!("Show Data Preview"), &mut next_show_data_preview);
                ...
                imgui::checkbox(imgui::str!("Grey out zeroes"), &mut self.opts.grey_out_zeroes);
            ...
            if self.opts.uppercase_hex {
                imgui::text(imgui::str_gbuf!("{:0digits$X} ... {:0digits$X}", visible_start_addr, visible_end_addr, digits=sizes.addr_digits_count as usize));
            } else {
                imgui::text(imgui::str_gbuf!("{:0digits$x} ... {:0digits$x}", visible_start_addr, visible_end_addr, digits=sizes.addr_digits_count as usize));
            }
            ...
            imgui::push_item_width((sizes.addr_digits_count + 1) as f32 * sizes.glyph_width + style.FramePadding.x * 2.0);
            {
                ...
                if imgui::input_text(imgui::str!("##addr"),
                    ...
                    if let Ok(Ok(goto_addr)) =  addr_input_buf.to_str().map(|s| u32::from_str_radix(s, 16)) {
        ...
        if self.opts.show_data_preview {
            ...
            imgui::same_line(0.0);
            imgui::push_item_width((sizes.glyph_width * 10.0) + style.FramePadding.x * 2.0 + style.ItemInnerSpacing.x);

            if imgui::begin_combo(imgui::str!("##combo_type"), self.preview_data_type.name(), imgui::ComboFlags::HeightLargest) {
                for data_type in DataType::VARIANTS.iter() {
                    if imgui::selectable(data_type.name(), self.preview_data_type == *data_type, imgui::none(), None) {
            ...
            imgui::same_line(0.0);
            imgui::push_item_width((sizes.glyph_width * 6.0) + style.FramePadding.x * 2.0 + style.ItemInnerSpacing.x);
            if imgui::begin_combo(imgui::str!("##combo_endianess"), self.preview_endianess.name(), imgui::ComboFlags::HeightLargest) {
                if imgui::selectable(Endian::Little.name(), self.preview_endianess == Endian::Little, imgui::none(), None) {
                    self.preview_endianess = Endian::Little;
                }
                if imgui::selectable(Endian::Big.name(), self.preview_endianess == Endian::Big, imgui::none(), None) {
            ...
            imgui::same_line(x);
            Self::text_data_as_dec(self.cursor_addr, self.preview_data_type, self.preview_endianess == Endian::Big, memory);

            imgui::text(imgui::str!("Hex"));
            imgui::same_line(x);
            Self::text_data_as_hex(self.cursor_addr, self.preview_data_type, self.preview_endianess == Endian::Big, memory);

            imgui::text(imgui::str!("Bin"));
            imgui::same_line(x);
            Self::text_data_as_bin(self.cursor_addr, self.preview_data_type, self.preview_endianess == Endian::Big, memory);
        }
    ...
    fn text_data_as_dec(addr: u32, data_type: DataType, big_endian: bool, memory: &dyn ArmMemory) {
        ...
        match data_type {
            DataType::S8    => imgui::text_unformatted(imgui::str_gbuf!("{}", set_endian!(memory.view_byte(addr) as i8))),
            DataType::U8    => imgui::text_unformatted(imgui::str_gbuf!("{}", set_endian!(memory.view_byte(addr)))),
            DataType::S16   => imgui::text_unformatted(imgui::str_gbuf!("{}", set_endian!(memory.view_halfword(addr) as i16))),
            DataType::U16   => imgui::text_unformatted(imgui::str_gbuf!("{}", set_endian!(memory.view_halfword(addr)))),
            DataType::S32   => imgui::text_unformatted(imgui::str_gbuf!("{}", set_endian!(memory.view_word(addr) as i32))),
            DataType::U32   => imgui::text_unformatted(imgui::str_gbuf!("{}", set_endian!(memory.view_word(addr)))),
            ...
            DataType::Double => {
                let lo = memory.view_word(addr);
                let hi = memory.view_word(addr.wrapping_add(4));
                let v = unsafe { std::mem::transmute::<u64, f64>(set_endian!((lo as u64) | ((hi as u64) << 32))) };
    ...
    fn text_data_as_hex(addr: u32, data_type: DataType, big_endian: bool, memory: &dyn ArmMemory) {
        ...
        match data_type {
            DataType::S8    => imgui::text_unformatted(imgui::str_gbuf!("{:02X}", set_endian!(memory.view_byte(addr) as i8))),
            DataType::U8    => imgui::text_unformatted(imgui::str_gbuf!("{:02X}", set_endian!(memory.view_byte(addr)))),
            DataType::S16   => imgui::text_unformatted(imgui::str_gbuf!("{:04X}", set_endian!(memory.view_halfword(addr) as i16))),
            DataType::U16   => imgui::text_unformatted(imgui::str_gbuf!("{:04X}", set_endian!(memory.view_halfword(addr)))),
            DataType::S32   => imgui::text_unformatted(imgui::str_gbuf!("{:08X}", set_endian!(memory.view_word(addr) as i32))),
            DataType::U32   => imgui::text_unformatted(imgui::str_gbuf!("{:08X}", set_endian!(memory.view_word(addr)))),
            ...
            DataType::Float => imgui::text_unformatted(imgui::str_gbuf!("{:08X}", set_endian!(memory.view_word(addr) as i32))),
            DataType::Double => {
                let lo = memory.view_word(addr);
                let hi = memory.view_word(addr.wrapping_add(4));
                let v = set_endian!((lo as u64) | ((hi as u64) << 32));
    ...
    fn text_data_as_bin(addr: u32, data_type: DataType, big_endian: bool, memory: &dyn ArmMemory) {
        ...
        match data_type {
            DataType::S8    => imgui::text_unformatted(imgui::str_gbuf!("{:08b}", set_endian!(memory.view_byte(addr) as i8))),
            DataType::U8    => imgui::text_unformatted(imgui::str_gbuf!("{:08b}", set_endian!(memory.view_byte(addr)))),
            DataType::S16   => imgui::text_unformatted(imgui::str_gbuf!("{:016b}", set_endian!(memory.view_halfword(addr) as i16))),
            DataType::U16   => imgui::text_unformatted(imgui::str_gbuf!("{:016b}", set_endian!(memory.view_halfword(addr)))),
            DataType::S32   => imgui::text_unformatted(imgui::str_gbuf!("{:032b}", set_endian!(memory.view_word(addr) as i32))),
            DataType::U32   => imgui::text_unformatted(imgui::str_gbuf!("{:032b}", set_endian!(memory.view_word(addr)))),
            ...
            DataType::Float => imgui::text_unformatted(imgui::str_gbuf!("{:032b}", set_endian!(memory.view_word(addr) as i32))),
            DataType::Double => {
                let lo = memory.view_word(addr);
                let hi = memory.view_word(addr.wrapping_add(4));
                let v = set_endian!((lo as u64) | ((hi as u64) << 32));
...
impl DataType {
    ...
    pub const VARIANTS: [DataType; DataType::COUNT] = [
        DataType::S8, DataType::U8, DataType::S16, DataType::U16,
        DataType::S32, DataType::U32, DataType::S64, DataType::U64,
