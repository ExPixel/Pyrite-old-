...
pub struct OBJRender;

impl CBool for OBJWindow {
    const BOOL: bool = true;
}
impl CBool for OBJRender {
...
impl CBool for BitmapMode {
    const BOOL: bool = true;
}
impl CBool for TileMode {
    const BOOL: bool = false;
}

pub fn process_objs<IsOBJWindowMode: CBool, IsBitmapMode: CBool>(
    ...
    if !IsOBJWindowMode::BOOL
        && (pixels.windows.enabled && !pixels.windows.line_visible(Layer::OBJ))
    {
        return;
    }
    ...
    for obj_index in objects.iter().map(|x| (*x & 0xFF) as usize) {
        ...
        let mut pflags = if IsOBJWindowMode::BOOL {
            0
        } else {
        ...
        if attrs.0.palette256() {
            ...
            for obj_screen_draw in (obj_screen_left as usize)..=(obj_screen_right as usize) {
                ...
                {
                    let obj_x_i =
                        apply_mosaic_cond(attrs.0.mosaic(), obj_x.integer() as u16, mosaic_x)
                            as usize;
                    let obj_y_i =
                        apply_mosaic_cond(attrs.0.mosaic(), obj_y.integer() as u16, mosaic_y)
                            as usize;
