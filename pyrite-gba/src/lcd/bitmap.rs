...
use super::{LCDLineBuffer, LCDRegisters};
...
pub fn render_mode3(
    ...
    if registers.dispcnt.display_layer(4) {
        if bg2_priority < 3 {
            render_objects_placeholder(
                object_priorities.objects_with_priority(3),
                vram,
                oam,
                pal,
                pixels,
        ...
        if bg2_priority < 2 {
            render_objects_placeholder(
                object_priorities.objects_with_priority(2),
                vram,
                oam,
                pal,
                pixels,
        ...
        if bg2_priority < 1 {
            render_objects_placeholder(
                object_priorities.objects_with_priority(1),
                vram,
                oam,
                pal,
                pixels,
    ...
    if registers.dispcnt.display_layer(4) {
        ...
        render_objects_placeholder(
            object_priorities.objects_with_priority(0),
            vram,
            oam,
            pal,
            pixels,
...
#[inline(never)]
fn render_objects_placeholder(
    objects: &[u16],
    _vram: &VRAM,
    _oam: &OAM,
    _pal: &GbaPalette,
    _pixels: &mut LCDLineBuffer,
) {
}
